...
import { minorMajorProjects } from "../../../utils/data/courses/minorMajorProjects";
                    ...
                    {minorMajorProjects?.slice(0, 7)?.map((item, index) => (
                        <p
                            ...
                            className={`w-full flex items-start justify-start gap-3 text-sm cursor-pointer transition-all duration-300 pb-4 pt-2 border-b border-b-[#e4e6e9] ${
                                activePoint === index ? "text-black font-normal" : "text-[#616263] font-light"
                            }`}
                        >
                            <span
                                className={`bg-capabl_primary rounded-full flex-shrink-0 transition-all duration-300 transform ${
                                    activePoint === index ? "scale-100 opacity-100 h-2.5 md:h-3 w-2.5 md:w-3 mt-1" : "scale-0 opacity-0 h-0 w-0"
                                }`}
                            />{" "}
                            {item?.title}
                        </p>
                    ...
                    {viewAll &&
                        minorMajorProjects?.slice(7)?.map((item, index) => (
                            <p
                                ...
                                key={index + 7}
                                className={`w-full flex items-start justify-start gap-3 text-sm cursor-pointer transition-all duration-300 pb-4 pt-2 border-b border-b-[#e4e6e9] ${
                                    activePoint === index + 7 ? "text-black font-normal" : "text-[#616263] font-light"
                                }`}
                            >
                                <span
                                    className={`bg-capabl_primary rounded-full flex-shrink-0 transition-all duration-300 transform ${
                                        activePoint === index + 7 ? "scale-100 opacity-100 h-2.5 md:h-3 w-2.5 md:w-3 mt-1" : "scale-0 opacity-0 h-0 w-0"
                                    }`}
                                />{" "}
                                {item?.title}
                            </p>
