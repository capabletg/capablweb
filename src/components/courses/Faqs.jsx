            ...
            <div className="mt-8 md:mt-10">
                {!!filteredFaqs?.length &&
                    filteredFaqs?.map(({ question, answer }, index) => (
                        <div key={index} className={`${filteredFaqs?.length - 1 !== index && "border-b border-b-[#e4e4e4]"} py-3 sm:py-4 md:py-5`}>
                            <div
                                onClick={() => setunWrapIndex((prev) => (prev === index ? null : index))}
                                className="cursor-pointer flex items-start justify-between"
                            >
                                <p className="text-sm md:text-[0.92rem] font-medium text-black">{question}</p>
                                <span>
                                    <HiOutlineChevronDown className={`text-base md:text-xl ${unWrapIndex === index ? "rotate-180" : "rotate-0"}`} />
                                </span>
                            </div>
                            <div
                                className={`text-xs md:text-sm font-light text-[#565656] overflow-hidden pr-6 ${
                                    unWrapIndex === index ? "opacity-100 max-h-screen mt-2.5 mb-1" : "opacity-0 max-h-0 mt-0"
                                }`}
                            >
                                {answer}
                            </div>
                ...
                {!filteredFaqs?.length && <p className="text-center font-light text-xs md:text-sm text-[#565656]">No Faqs Found</p>}
            </div>
        </div>
    );
}
